...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="conclave",
        description="Multi-agent consensus engine — get a second AI opinion without being human middleware.",
    )
    parser.add_argument("--version", action="version", version="%(prog)s 0.1.0")
    sub = parser.add_subparsers(dest="command")

    models = sub.add_parser("models", help="List, check status, or benchmark configured models")
    models_sub = models.add_subparsers(dest="models_cmd")
    models_sub.add_parser("list", help="Show all configured models")
    models_sub.add_parser("status", help="Ping each model and report availability")
    models_sub.add_parser("benchmark", help="Run latency benchmarks against each model")

    plan = sub.add_parser("plan", help="Generate a role-assignment plan without running deliberation")
    plan.add_argument("--query", required=True, help="The question or topic to plan for")

    run = sub.add_parser("run", help="Run a full deliberation and return a verdict")
    run.add_argument("--query", required=True, help="The question or topic to deliberate on")
    run.add_argument("--collection", action="append", help="RAG collection to search (repeatable)")
    run.add_argument("--input-file", help="Path to a file whose contents are appended as context")
    run.add_argument("--output-md", help="Write the verdict to a Markdown file at this path")
    run.add_argument("--output-type", help="Structured output format: report|decision|plan|checklist|build_spec|image_brief|model_3d_brief")
    run.add_argument("--progress", action="store_true", help="Stream deliberation progress to stderr")
    run.add_argument("--max-seconds", type=int, help="Overall timeout for the entire run (seconds)")
    run.add_argument("--max-evidence", type=int, help="Maximum number of RAG evidence items to retrieve")
    run.add_argument("--max-context-chars", type=int, help="Truncate context to this many characters")
    ...
    iterate = sub.add_parser("iterate", help="Run consensus iteratively on a topic from a file")
    iterate.add_argument("--input-file", required=True, help="Path to file containing the topic/query")
    iterate.add_argument("--query", help="Override query (default: read from input file)")
    iterate.add_argument("--collection", action="append", help="RAG collection to search (repeatable)")
    iterate.add_argument("--output-dir", help="Directory to write per-run Markdown outputs")
    iterate.add_argument("--append-md", help="Append each verdict to this Markdown file")
    iterate.add_argument("--max-runs", type=int, help="Stop after this many iterations")
    iterate.add_argument("--sleep-seconds", type=float, default=600, help="Seconds between iterations (default: 600)")
    iterate.add_argument("--stop-on-insufficient", action="store_true", help="Stop iterating if evidence quality is insufficient")
    iterate.add_argument("--progress", action="store_true", help="Stream deliberation progress to stderr")
    iterate.add_argument("--max-seconds", type=int, help="Overall timeout per iteration (seconds)")
    iterate.add_argument("--max-evidence", type=int, help="Maximum number of RAG evidence items to retrieve")
    iterate.add_argument("--max-context-chars", type=int, help="Truncate context to this many characters")
    ...
    runs_sub.add_parser("latest", help="Show the most recent run")
    list_cmd = runs_sub.add_parser("list", help="List recent runs")
    list_cmd.add_argument("--limit", type=int, default=10, help="Number of runs to show (default: 10)")

    reconcile = sub.add_parser("reconcile", help="Re-summarize past runs on a topic into one verdict")
    reconcile.add_argument("--topic", required=True, help="Topic string to match against past runs")
    reconcile.add_argument("--fail-on-insufficient", action="store_true", help="Exit non-zero if evidence quality is insufficient")
    reconcile.add_argument("--no-fail-on-insufficient", action="store_true", help="Never exit non-zero for evidence quality")

    sub.add_parser("index", help="Build or refresh the RAG search index")

    audit = sub.add_parser("audit", help="Audit RAG sources, MCP connections, and data quality")
    audit.add_argument("--mode", choices=["all", "rag", "mcp", "sources"], default="all", help="What to audit (default: all)")
    audit.add_argument("--output-dir", help="Write audit report files to this directory")
    audit.add_argument("--no-fetch", action="store_true", help="Skip fetching live data (use cached only)")
    audit.add_argument("--fail-on-issues", action="store_true", help="Exit non-zero if any audit issues are found")

    sub.add_parser("health", help="Check that all configured models are reachable")

    validate = sub.add_parser("validate", help="Validate config, model health, and query routing")
    validate.add_argument("--routing", action="store_true", help="Only validate query routing rules")
    validate.add_argument("--health", action="store_true", help="Only validate model health")
    validate.add_argument("--config", action="store_true", help="Only validate config file syntax")

    sub.add_parser("validate-routing", help="Validate query routing rules (shortcut for validate --routing)")

    eval_cmd = sub.add_parser("eval", help="Run evaluation cases comparing Conclave vs a single-model baseline")
    eval_cmd.add_argument("--case-id", action="append", help="Run only these evaluation case IDs (repeatable)")
    eval_cmd.add_argument("--limit", type=int, help="Maximum number of cases to run")
    eval_cmd.add_argument("--baseline-model", help="Model ID for the single-model baseline (default: from config)")
    eval_cmd.add_argument("--judge-model", help="Model ID for the judge that scores answers (default: from config)")
    eval_cmd.add_argument("--no-judge", action="store_true", help="Skip judging, just run both answers")
    eval_cmd.add_argument("--output-json", help="Write evaluation results to this JSON file")

    schedule = sub.add_parser("schedule", help="Manage systemd timers for scheduled runs")
    schedule_sub = schedule.add_subparsers(dest="schedule_cmd")
    schedule_sub.add_parser("list", help="Show configured schedules")
    apply_cmd = schedule_sub.add_parser("apply", help="Install or update systemd timer units")
    apply_cmd.add_argument("--unit-dir", default="~/.config/systemd/user", help="Directory for systemd unit files")
    apply_cmd.add_argument("--enable", action="store_true", help="Enable and start timers after installing")
    apply_cmd.add_argument("--no-reload", action="store_true", help="Skip systemctl daemon-reload")
    apply_cmd.add_argument("--dry-run", action="store_true", help="Print what would be done without writing files")
    apply_cmd.add_argument("--no-validate", action="store_true", help="Skip config validation before applying")
    apply_cmd.add_argument("--disable-legacy", action="store_true", help="Disable old timer units replaced by this config")
...
def _progress_printer(store: DecisionStore, run_id: str, stop_event: threading.Event) -> None:
    ...
    while not stop_event.is_set():
        ...
        for event in events[last:]:
            ...
            detail = ""
            if phase == "preflight" and status == "warnings":
