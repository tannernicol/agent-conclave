...
const uiState = window.ConclaveUI || {};
const buildPhaseState = uiState.buildPhaseState || (() => []);
const summarizeEvent = uiState.summarizeEvent || (() => '');
const wsClient = window.ConclaveWS || null;
  ...
  promptListEl.innerHTML = sorted.map((prompt) => {
    ...
    return `
      <div class="prompt-card${selectedClass}" data-prompt-id="${escapeHtml(prompt.id)}" data-run-id="${escapeHtml(latest?.id || '')}">
        ...
        ${latest?.id ? `<button class="ui-button ghost small" data-action="view" data-run-id="${escapeHtml(latest.id)}">Open</button>` : ''}
...
async function pollRun(runId) {
  ...
  if (wsClient?.openRunSocket) {
    runWebSocket = wsClient.openRunSocket({
      runId,
      onState: (run) => {
      ...
      onFallback: () => {
        setRunActionEphemeral('Live updates lost', 'Reconnecting failed; switching to polling.', 'warning', 4000);
        startPolling(runId);
      },
      onStatus: (info) => {
        if (info.state === 'retrying') {
          setRunActionEphemeral('Reconnecting live updates', `Attempt ${info.retries}.`, 'warning', 1500);
        }
      },
    });
    return;
